# Graph Transformed Network based on transformer NN and GCN
import torch
...
class GTN(nn.Module):
  ...
  def __init__(self, in_dim=1, hidden_dim=16, out_dim=1, n_head=1, dropout=0.1, num_GC_layers=1):
    ...
    if self.num_GC_layers == 1:
        self.gc1 = GCNConv(in_dim, out_dim)
  ...
  def forward(self, x, adj, nodes_u, nodes_v):
    ...
    # TODO: combine Transformer with GTN by transforming GCN (like in line 54) into passable value for transformer 
